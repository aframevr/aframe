...
module.exports.System = registerSystem('texture', {
  ...
  loadImage: function (el, material, data, src) {
    ...
    if (cache[src] && cache[src][repeat]) {
      cache[src][repeat].then(handleImageTextureLoaded);
      return;
    }

    // Material instance is first to try to load this texture. Load it.
    cache[srcString] = cache[srcString] || {};
    cache[srcString][repeat] = cache[srcString][repeat] || {};
    cache[srcString][repeat] = loadImageTexture(material, src, repeat);
    cache[srcString][repeat].then(handleImageTextureLoaded);
