...
var app = budo('./index.js:build/vr-components.js', {
...
.on('watch', function (eventType, fn) {
  ...
  if (path.extname(fn) === '.css') {
    ...
    app.reload(fn);
  } else if (fn.indexOf('core/templates') === 0) {
    var templatesWriteStream = fs.createWriteStream('./build/vr-components.html');
    var p = exec('vulcanize --inline core/templates/index.html').stdout;
    p.pipe(templatesWriteStream);
    p.on('end', function () {
      app.reload('build/vr-components.html');
    });
