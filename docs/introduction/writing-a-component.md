...
Because they happen so often, the `.tick()` and `.tock()` calls are quite time-sensitive, so every effort to improve performance within the handler is valuable.  Here is a refactored version of the above which reduces the time spent handling the event:
...
Since `factor * (timeDelta / 1000)` doesn't change until the next `.tick()`, we only need to calculate it once:

```js
    var deltaImpact = factor * (timeDelta / 1000);
   
```

Creating and iterating an array takes extra time, as does looking up `directionVec3[axis]`.  We can remove those expenses and perform the calculation as part of the calculation for the new position:

```js    
    // Translate the entity in the direction towards the target.
    this.el.setAttribute('position', {
      x: currentPosition.x + (directionVec3.x *= deltaImpact),
      y: currentPosition.y + (directionVec3.y *= deltaImpact),
      z: currentPosition.z + (directionVec3.z *= deltaImpact)
    });
  }
});
```
